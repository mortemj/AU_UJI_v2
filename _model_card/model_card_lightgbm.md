...
- **Definición exacta de la variable objetivo:** **(verificar)** — no consta el
  criterio literal (p. ej. número de cursos sin matrícula) en
  `metricas_modelo.json`; debe tomarse de la fuente de definición del *target*
  del proyecto (fase 3) y no deducirse aquí.
...
- **Definición de la variable objetivo:** debe documentarse desde la fuente del
  proyecto **(verificar)**.
